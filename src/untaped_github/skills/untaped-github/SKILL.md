...
- Default freshness refreshes uncached, stale, or under-profiled repos; `--refresh` forces preparation, while `--cached` makes no network calls and rejects `--team`. A failed refresh scans a covering cached copy; otherwise it becomes a declared failure. Every format reports the summary and failures on stderr.
